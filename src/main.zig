...
pub fn main() !void {
    var gpa_creator = std.heap.GeneralPurposeAllocator(.{}).init;
    defer std.debug.assert(gpa_creator.deinit() == .ok);
    const gpa: std.mem.Allocator = gpa_creator.allocator();
    ...
    //                 const ret: c_int = nc.ncplane_cursor_move_yx(content_plane, -1, @intCast(cursor_x + 1));
    //                 if (ret != 0) {
